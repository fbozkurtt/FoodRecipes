...
namespace FoodRecipes.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetStoreLocation(bool? useSsl = null)
        {
            ...
            //get store host
            var host = GetStoreHost(useSsl ?? IsCurrentConnectionSecured());
            if (!string.IsNullOrEmpty(host))
            {
                //add application path base if exists
                storeLocation = IsRequestAvailable() ? $"{host.TrimEnd('/')}{_httpContextAccessor.HttpContext.Request.PathBase}" : host;
            }
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[FoodRecipesHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[FoodRecipesHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[FoodRecipesHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
