...
using FoodRecipes.Data.DataProviders.LinqToDb;
using LinqToDB;
...
namespace FoodRecipes.Data.DataProviders
{
    public class PostgreSqlDataProvider : BaseDataProvider, IFoodRecipesDataProvider
    {
        #region Fields
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new FoodRecipesException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new FoodRecipesException("A table's primary key does not have an identity constraint");
        ...
        public void InitializeDatabase()
        {
            ...
            migrationManager.ApplyUpMigrations(typeof(FoodRecipesDbStartup).Assembly);
        }
        ...
        /// </summary>
        /// <param name="foodRecipesConnectionString">Connection string info</param>
        /// <returns>Connection string</returns>
        public virtual string BuildConnectionString(IFoodRecipesConnectionStringInfo foodRecipesConnectionString)
        {
            if (foodRecipesConnectionString is null)
                throw new ArgumentNullException(nameof(foodRecipesConnectionString));

            if (foodRecipesConnectionString.IntegratedSecurity)
                throw new FoodRecipesException("Data provider supports connection only with login and password");

            var builder = new NpgsqlConnectionStringBuilder
            {
                Host = foodRecipesConnectionString.ServerName,
                //Cast DatabaseName to lowercase to avoid case-sensitivity problems
                Database = foodRecipesConnectionString.DatabaseName.ToLowerInvariant(),
                Username = foodRecipesConnectionString.Username,
                Password = foodRecipesConnectionString.Password,
            };
