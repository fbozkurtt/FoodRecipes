...
namespace FoodRecipes.Services.Localization
{
    /// <summary>
    ...
    public partial class LocalizationService : ILocalizationService
    {
        ...
        protected virtual async Task<IDictionary<string, string>> UpdateLocaleResourceAsync(IDictionary<string, string> resources, int? languageId = null, bool clearCache = true)
        {
            ...
            //clear cache
            if (clearCache)
                await _staticCacheManager.RemoveByPrefixAsync(FoodRecipesEntityCacheDefaults<LocaleStringResource>.Prefix);
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(FoodRecipesLocalizationDefaults.LocaleStringResourcesAllCacheKey, languageId);
            ...
            if (!loadPublicLocales.HasValue || allLocales != null)
            {
                ...
                await _staticCacheManager.RemoveAsync(FoodRecipesLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey, languageId);
                await _staticCacheManager.RemoveAsync(FoodRecipesLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey, languageId);
            ...
            key = _staticCacheManager.PrepareKeyForDefaultCache(loadPublicLocales.Value
                ? FoodRecipesLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey
                : FoodRecipesLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey,
                languageId);
            ...
            return await _staticCacheManager.GetAsync(key, () =>
            {
                ...
                var query = from l in _lsrRepository.Table
                            ...
                            where l.LanguageId == languageId
                            select l;
                query = loadPublicLocales.Value ? query.Where(r => !r.ResourceName.StartsWith(FoodRecipesLocalizationDefaults.AdminLocaleStringResourcesPrefix)) : query.Where(r => r.ResourceName.StartsWith(FoodRecipesLocalizationDefaults.AdminLocaleStringResourcesPrefix));
            ...
            bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
        {
            ...
            if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
            {
                ...
                var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(FoodRecipesLocalizationDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
            ...
            {
                //gradual loading
                var key = _staticCacheManager.PrepareKeyForDefaultCache(FoodRecipesLocalizationDefaults.LocaleStringResourcesByNameCacheKey
        ...
        public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
        {
            ...
            await xmlWriter.WriteAttributeStringAsync("SupportedVersion", FoodRecipesVersion.CURRENT_VERSION);
        ...
        public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, int? languageId = null) where TEnum : struct
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{FoodRecipesLocalizationDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
        ...
        public virtual async Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, int? languageId = null)
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{FoodRecipesLocalizationDefaults.PermissionLocaleStringResourcesPrefix}{permissionRecord.SystemName}";
            var result = await GetResourceAsync(resourceName, languageId ?? workingLanguage.Id, false, string.Empty, true);
