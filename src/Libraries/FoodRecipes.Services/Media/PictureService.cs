...
namespace FoodRecipes.Services.Media
{
    /// <summary>
    /// Picture service
    /// </summary>
    public partial class PictureService : IPictureService
    {
        ...
        private readonly IRepository<RecipePicture> _recipePictureRepository;
        ...
        public PictureService(IDownloadService downloadService,
            ...
            IRepository<RecipePicture> recipePictureRepository,
            ...
            MediaSettings mediaSettings)
        {
            ...
            _recipePictureRepository = recipePictureRepository;
        ...
        protected virtual async Task DeletePictureThumbsAsync(Picture picture)
        {
            ...
            var currentFiles = _fileProvider.GetFiles(_fileProvider.GetAbsolutePath(FoodRecipesMediaDefaults.ImageThumbsPath), filter, false);
        ...
        {
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath(FoodRecipesMediaDefaults.ImageThumbsPath);

            if (_mediaSettings.MultipleThumbDirectories)
            {
                //get the first two letters of the file name
                var fileNameWithoutExtension = _fileProvider.GetFileNameWithoutExtension(thumbFileName);
                if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > FoodRecipesMediaDefaults.MultipleThumbDirectoriesLength)
                {
                    var subDirectoryName = fileNameWithoutExtension[0..FoodRecipesMediaDefaults.MultipleThumbDirectoriesLength];
                    thumbsDirectoryPath = _fileProvider.GetAbsolutePath(FoodRecipesMediaDefaults.ImageThumbsPath, subDirectoryName);
            ...
            string storeLocation = null)
        {
            ...
            {
                PictureType.Avatar => await _settingService.GetSettingByKeyAsync("Media.Customer.DefaultAvatarImageName", FoodRecipesMediaDefaults.DefaultAvatarFileName),
                _ => await _settingService.GetSettingByKeyAsync("Media.DefaultImageName", FoodRecipesMediaDefaults.DefaultImageFileName),
            };
        ...
        /// </returns>
        public virtual async Task<IList<Picture>> GetPicturesByRecipeIdAsync(int recipeId, int recordsToReturn = 0)
        {
            if (recipeId == 0)
                return new List<Picture>();

            var query = from p in _pictureRepository.Table
                        join pp in _recipePictureRepository.Table on p.Id equals pp.PictureId
                        orderby pp.DisplayOrder, pp.Id
                        where pp.RecipeId == recipeId
                        select p;
        ...
        /// The task result contains the picture
        /// </returns>
        public virtual async Task<Picture> GetRecipePictureAsync(Recipe recipe)
        {
            if (recipe == null)
                throw new ArgumentNullException(nameof(recipe));

            //now let's load the default product picture
            var productPicture = (await GetPicturesByRecipeIdAsync(recipe.Id, 1)).FirstOrDefault();
