using System.Threading.Tasks;
using FoodRecipes.Core.Domain.Seo;
using FoodRecipes.Services.Caching;
using Nop.Services.Seo;

namespace FoodRecipes.Services.Seo.Caching
{
    /// <summary>
    /// Represents an URL record cache event consumer
    /// </summary>
    public partial class UrlRecordCacheEventConsumer : CacheEventConsumer<UrlRecord>
    {
        /// <summary>
        /// Clear cache data
        /// </summary>
        /// <param name="entity">Entity</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        protected override async Task ClearCacheAsync(UrlRecord entity)
        {
            await RemoveAsync(FoodRecipesSeoDefaults.UrlRecordCacheKey, entity.EntityId, entity.EntityName, entity.LanguageId);
            await RemoveAsync(FoodRecipesSeoDefaults.UrlRecordBySlugCacheKey, entity.Slug);
        }
    }
}
