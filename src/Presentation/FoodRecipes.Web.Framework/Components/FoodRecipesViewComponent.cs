...
namespace FoodRecipes.Web.Framework.Components
{
    /// <summary>
    /// Base class for ViewComponent in nopCommerce
    /// </summary>
    public abstract class FoodRecipesViewComponent : ViewComponent
    {
        private void PublishModelPrepared<TModel>(TModel model)
        {
            ...
            if (model is BaseFoodRecipesModel)
            {
                ...
                eventPublisher.ModelPreparedAsync(model as BaseFoodRecipesModel).Wait();
            }

            if (model is IEnumerable<BaseFoodRecipesModel> modelCollection)
            {
                var eventPublisher = EngineContext.Current.Resolve<IEventPublisher>();
