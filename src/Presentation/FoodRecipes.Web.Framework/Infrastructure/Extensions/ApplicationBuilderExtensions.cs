...
using FoodRecipes.Data.Migrations;
...
namespace FoodRecipes.Web.Framework.Infrastructure.Extensions
{
    ...
    public static class ApplicationBuilderExtensions
    {
        ...
        {
            var fileProvider = EngineContext.Current.Resolve<IFoodRecipesFileProvider>();
            var appSettings = EngineContext.Current.Resolve<AppSettings>();
            ...
            application.UseStaticFiles(new StaticFileOptions
            {
                FileProvider = new PhysicalFileProvider(fileProvider.GetAbsolutePath(FoodRecipesCommonDefaults.DbBackupsPath)),
            ...
            if (DataSettingsManager.IsDatabaseInstalled())
            {
                application.UseStaticFiles(new StaticFileOptions
                {
                    FileProvider = new RoxyFilemanProvider(fileProvider.GetAbsolutePath(FoodRecipesRoxyFilemanDefaults.DefaultRootDirectory.TrimStart('/').Split('/'))),
                    RequestPath = new PathString(FoodRecipesRoxyFilemanDefaults.DefaultRootDirectory),
                    OnPrepareResponse = staticFileResponse
                });
            }
        ...
        public static void UseNopRequestLocalization(this IApplicationBuilder application)
        {
            application.UseRequestLocalization(async options =>
            {
                ...
                options.AddInitialRequestCultureProvider(new FoodRecipesSeoUrlCultureProvider());
                ...
                if (cookieRequestCultureProvider is not null)
                    cookieRequestCultureProvider.CookieName = $"{FoodRecipesCookieDefaults.Prefix}{FoodRecipesCookieDefaults.CultureCookie}";
            });
        }
        ...
        /// </summary>
        /// <param name="application">Builder for configuring an application's request pipeline</param>
        public static void UseFoodRecipesWebMarkupMin(this IApplicationBuilder application)
        {
            //check whether database is installed
            if (!DataSettingsManager.IsDatabaseInstalled())
                return;
